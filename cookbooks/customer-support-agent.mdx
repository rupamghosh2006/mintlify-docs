...
> **Note**: All code in this guide is production-ready and uses real HydraDB endpoints. Base URL: `https://api.hydradb.com`. Get your API key at [app.hydradb.com](app.hydradb.com).
...
async function recallCustomerContext(
    ...
    const kbData = await client.recall.fullRecall({
        tenant_id:     TENANT_ID,
        sub_tenant_id: KB_SUB_TENANT,
        query:         customerMsg,
    ...
    const memData = await client.recall.recallPreferences({
        tenant_id:     TENANT_ID,
        sub_tenant_id: `customer-${customerId}`,
        query:         customerMsg,
        max_results:   8,
...
def recall_customer_context(
    ...
    """
    Two-call recall pattern:
    1. full_recall        - searches knowledge base (docs, resolved tickets)
    2. recall_preferences - searches customer's personal memory
    Merge both before passing to LLM.
    mode: "thinking" enables personalised ranking on both calls.
    sub_tenant_id scopes each call to the right data store.
    """
    # Call 1: knowledge base - help docs, resolved tickets, FAQs
    kb_data = client.recall.full_recall(
        tenant_id=TENANT_ID,
        sub_tenant_id=KB_SUB_TENANT,
        query=customer_msg,
    ...
    # Call 2: customer personal memory - preferences, history, account facts
    mem_data = client.recall.recall_preferences(
        tenant_id=TENANT_ID,
        sub_tenant_id=customer_sub_tenant(customer_id),
        query=customer_msg,
        max_results=8,
...
async function escalateToHuman(
    ...
    const customerProfile = await client.recall.recallPreferences({
        tenant_id:     TENANT_ID,
        sub_tenant_id: `customer-${customerId}`,
    ...
    const similarData = await client.recall.fullRecall({
        tenant_id:   TENANT_ID,
...
def escalate_to_human(
    ...
    """
    # Recall full customer memory profile
    customer_profile = client.recall.recall_preferences(
        tenant_id=TENANT_ID,
        sub_tenant_id=customer_sub_tenant(customer_id),
        query="customer account history preferences past issues plan",
    )
    ...
    # Recall similar past tickets resolved by humans
    similar_data = client.recall.full_recall(
        tenant_id=TENANT_ID,
