...
```bash
curl -X POST \
  'https://api.hydradb.com/source/status?tenant_id=competitive-intel&source_ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```
GET /source/status?tenant_id=competitive-intel&source_ids=YOUR_SOURCE_ID
Body: {}
```
