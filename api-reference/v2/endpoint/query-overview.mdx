...
| <Field name="sub_tenant_ids" /> | `string[]` or weighted object | Query one or more user/workspace/team scopes. A list uses equal normalized weights; an object like `{ "workspace_42": 2, "user_alex": 1 }` applies relative ranking weights. Max 100 sub-tenants. |
...
| Personalized answer | `type="all"`, include `sub_tenant_ids=["user_id"]`, `query_by="hybrid"`, `mode="thinking"` |
| User preferences only | `type="memory"`, include `sub_tenant_ids=["user_id"]`, `query_by="hybrid"` |
...
Use this when the answer should combine shared knowledge with user-specific context. Pass the same scope used at memory ingestion via `sub_tenant_ids: ["user_alex"]`.
...
Use this when the same question should search several sub-tenant scopes and return one globally ranked result set. Use a list for equal weighting, or an object when one scope should influence ranking more strongly.
