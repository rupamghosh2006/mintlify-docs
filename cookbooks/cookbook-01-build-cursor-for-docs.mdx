...
> **All code in this cookbook is real.** Base URL is `https://api.hydradb.com`. Get your API key at [hydradb.com](https://hydradb.com) or email [founders@hydradb.com](mailto:founders@hydradb.com).
...
def _poll_until_ready(timeout=180, interval=4):
    ...
    while time.time() < deadline:
        ...
        s = status.get("status", "")
        if   s == "ready":  print("✓ Tenant ready."); return
        elif s == "failed": raise RuntimeError(f"Provisioning failed: {status}")
        print(f"  status={s} - retrying in {interval}s")
        time.sleep(interval)
...
**What:** Switches to **JSON body ingestion** via `/ingestion/upload_knowledge` with `app_knowledge` field - the advanced ingestion path. This gives you full control over IDs, timestamps, collections, metadata, and (in Phase 2) explicit graph `relations`. The `{repo_name}/{relative_path}` ID convention is required for Phase 2 graph linking.
...
def upload_batch(batch: list) -> list:
    ...
    ids = [r["source_id"] for r in result.get("results", []) if r.get("source_id")]
...
def build_context(chunks: list, min_score: float = 0.5) -> tuple[str, list]:
    ...
    for chunk in chunks:
        ...
        seen.add(uid)
        doc_type = chunk.get("additional_metadata", {}).get("doc_type", "doc")
...
**Fix:** Correct paths: `/ingestion/upload_knowledge` (file upload or JSON batch with `app_knowledge`), `/recall/full_recall`, `/ingestion/verify_processing`. If using `sub_tenant_id`, confirm at least one batch was successfully uploaded to that sub-tenant first.
...
**POST** `/ingestion/upload_knowledge` - Max 20 items · body field `app_knowledge` (JSON string) · supports relations
