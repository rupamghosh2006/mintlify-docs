...
The SDKs return the inner `data` payload from `POST /query`. Raw HTTP responses wrap the same payload under `data` inside the standard response envelope.
...
The simplest path is one `POST /query` with `type: "all"` - HydraDB searches both stores in parallel and returns one merged, ranked result set.
...
- [Memories](/essentials/v2/memories) - what `POST /query` with `type: "memory"` searches
- [Knowledge](/essentials/v2/knowledge) - what `POST /query` with `type: "knowledge"` searches
