...
> **Note**: All code in this guide uses the official HydraDB Python SDK. Install it with `pip install hydradb-sdk`. Base URL: `https://api.hydradb.com`. Get your API key at [hydradb.com](https://hydradb.com) or email founders@hydradb.com.
...
graph LR
    ...
    C["New hire question"] -->|"recall.full_recall()"| B
...
> Phase 0 complete. The same `client.recall.full_recall()` retrieval pattern is used in every later phase. Your application can pass returned chunks into an LLM to generate the final answer.
...
def ask_onboarding(hire_id: str, question: str, topic: str = "") -> str:
    ...
    company_context = client.recall.full_recall(
        tenant_id=TENANT_ID,
        query=question,
        max_results=5,
        mode="thinking",
    ...
    hire_context = client.recall.recall_preferences(
        tenant_id=TENANT_ID,
        sub_tenant_id=hire_sub(hire_id),
        query=question,
        max_results=3,
...
| `client.recall.full_recall(tenant_id=..., query=..., max_results=..., mode=...)` | Retrieve company context for a hire question |
...
### client.recall.full_recall - key parameters
