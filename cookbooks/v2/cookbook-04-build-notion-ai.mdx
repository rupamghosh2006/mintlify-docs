...
def upload_batch(sources: list, sub_tenant_id: str = None) -> list:
    """Upload up to 20 sources. Returns list of source IDs."""
    if sub_tenant_id:
        for s in sources:
...
**Endpoint:** `GET /source/status?source_ids=SOURCE_ID&tenant_id=TENANT` - Check indexing status

```python
def poll_until_indexed(source_id: str, timeout: int = 120, interval: int = 3):
    ...
    while time.time() < deadline:
        ...
        time.sleep(interval)
    raise TimeoutError(f"Indexing timed out for {source_id}")
...
**POST** `/source/status?source_ids=SOURCE_ID&tenant_id=YOUR_TENANT` - Poll until status = "completed"
