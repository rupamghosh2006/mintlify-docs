...
def ingest_people(people: list):
    ...
    for i, p in enumerate(people):
        ...
        metadata_list.append({
            "source_id": f"person-{p['name'].lower().replace(' ', '-')}",
...
| `file_metadata` | `str` | Optional. JSON **string** (not dict) with per-file `source_id` and `additional_metadata`. |
