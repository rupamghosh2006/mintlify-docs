...
- **Async indexing.** `202 Accepted` means HydraDB queued the work, not that content is searchable. Poll [Check indexing status](/api-reference/v2/endpoint/source-status) until `indexing_status` reaches `graph_creation` (searchable) or `completed` (graph-ready).
