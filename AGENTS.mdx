...
5. **`POST /search`** (`client.search.query()`): Retrieve context via `type: "knowledge"`, `"memory"`, or `"all"`.
...
The canonical request field for `POST /search` is `type`:
...
Use SDK autocomplete/returned object shape for the exact field spelling. For embeddings-only tenants (`is_embeddings_tenant: true`), `ready_for_ingestion` is not the right gate for `/source/ingest`; the memory vector store may remain false while raw `/embeddings/*` operations are usable.
...
| App search | `type: "knowledge"`, `search_by: "hybrid"`, `mode: "thinking"`, `search_apps: true`; still searches non-app knowledge too |
...
Use this only when the application owns embedding generation and chunking. Embeddings tenants are not intended for `/source/ingest`; their status may show `vectorstore_status.memories: false` even when raw embeddings operations are available.
...
| `client.search.query()` | `POST /search` | Unified search over knowledge, memories, or both |
...
| Search | `client.search.query()` | `POST /search` | Unified search over knowledge, memories, or both |
...
### 17.4 Raw HTTP: ingest app-source knowledge
