...
| --- | --- | --- |
| `sub_tenant_ids` | `string[]` or `{ [sub_tenant_id]: positive number }` | Preferred query-time scope selector. Use a single-item list for one user/workspace, a longer list to fan out with equal normalized weights, or an object to provide relative ranking weights. Maximum 100 sub-tenants. |
| `sub_tenant_id` | string | Legacy/deprecated query-time single-scope selector. Use `sub_tenant_ids: ["user_123"]` for new query calls. Ingest still uses singular `sub_tenant_id`. |
...
If you need to query several users, teams, or workspaces at once, pass `sub_tenant_ids`. A list gives every scope equal normalized weight; an object applies relative ranking weights before the final merged ranking:
...
- **Default to** `type: "all"` for personalized answers  -  one call instead of two, scoped with `sub_tenant_ids`.
...
**Cache the prompt context.** If the same query repeats inside a session, cache the `RetrievalResult` keyed by `(tenant_id, sub_tenant_ids, query, type, query_by)`. Recall is fast, but skipping it entirely is faster.
