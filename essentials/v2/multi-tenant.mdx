...
For personalized answers grounded in shared Knowledge, use the `type` parameter on `POST /search`:
...
When you need different formatting for shared vs personal context in the LLM prompt, call `POST /search` twice (once with `type: "knowledge"`, once with `type: "memory"`) and combine in your application.
...
For a personalized answer, the common pattern is one `POST /search` with `type: "all"` and the user's `sub_tenant_id`. HydraDB runs both stores in parallel and merges results. Use two separate calls only when you need to format the two streams differently in your LLM prompt.
