...
HydraDB uses the same top-level envelope for successful and failed requests.
...
| `FORBIDDEN` | `403` | The API key is valid but does not have access to the requested resource. |
...
result = with_retry(lambda: client.search.query(
    tenant_id="my_first_tenant",
    query="What are the pricing tiers?",
    type="knowledge",
))
...
- The `type` value may exclude the collection you need. Use `type: "all"` when combining knowledge and memories in `POST /search`.
...
- [Search](/api-reference/v2/endpoint/search) - retrieval parameters and response shape
