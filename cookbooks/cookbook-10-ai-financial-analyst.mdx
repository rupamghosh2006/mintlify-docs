...
def ingest_earnings_pdf(
    ...
    SDK params AND inside the app_sources JSON - AppKnowledgeModel validates both.
    """
...
def track_narrative_shift(
    ...
    chunks = sorted(
        recall.get("chunks", []),
        key=lambda c: c.get("document_metadata", {}).get("period_end_date", ""),
    )
    ...
    context_text = "\n\n".join(
        f"[{c['document_metadata'].get('period_label', '?')}]\n{c['chunk_content']}"
        for c in chunks
    )
...
def generate_quarterly_briefing(
    ...
    current_context = "\n\n".join(
        f"[{c.get('document_metadata', {}).get('period_label','?')} | "
        f"{c.get('document_metadata', {}).get('doc_type','?')}]\n{c['chunk_content']}"
...
```json
// app_sources - JSON string of the array below
[{
