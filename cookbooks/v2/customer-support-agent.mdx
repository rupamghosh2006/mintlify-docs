...
> **SDK note**: Install the official Python SDK with `pip install hydradb-sdk`. You can also call `POST /context/ingest` directly, but it accepts `multipart/form-data`, so JSON payloads such as `memories` and `app_knowledge` must be encoded as form fields. The Python import name differs from the package name.
