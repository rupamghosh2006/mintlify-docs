...
### Querying multiple sub-tenants

Use `sub_tenant_ids` when one query should fan out across multiple user, workspace, or team scopes. The field accepts either a list or a weighted object:

```json Equal weighting
{
  "tenant_id": "acme_corp",
  "sub_tenant_ids": ["workspace_42", "user_alex"],
...
A list gives every sub-tenant equal normalized weight. An object treats values as positive relative ranking weights and normalizes them server-side. You can send at most 100 sub-tenants. When `max_results` is omitted, HydraDB uses up to 10 results per sub-tenant, capped at 1000 fanout candidates before the final ranked response is shaped.
...
result = client.query(
    ...
    sub_tenant_ids=["your-sub-tenant"],
...
| <Field name="sub_tenant_ids" type='string[] or { [sub_tenant_id]: positive number }' /> | Preferred query-time sub-tenant selector. Send a list for equal normalized weights, or an object to fan out with relative ranking weights. Maximum 100 sub-tenants. |
| <Field name="sub_tenant_id" type="string or null" /> | Deprecated query-time single sub-tenant selector. Use `sub_tenant_ids: ["your-sub-tenant"]` instead. Ingest endpoints still use singular `sub_tenant_id`. |
...
- **Common mistakes.** Check [Ingestion Status](/api-reference/v2/endpoint/source-status) for recently ingested documents before querying. If you omit `sub_tenant_ids` and deprecated `sub_tenant_id`, HydraDB queries the default sub-tenant. Use [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants) to discover available IDs.
</Warning>
