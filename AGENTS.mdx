...
> **Important:** Prefer the v2 surface in this guide. Older v1 routes like `/ingestion/upload_knowledge`, `/list/data`, and `/tenants/create` may still appear in the OpenAPI file for compatibility, but new integrations should use the consolidated v2 endpoints: `/tenants`, `/context/*`, and `/query`.
...
const tenant_id = "my_first_tenant";

await client.tenants.create({ tenant_id: tenant_id });

while (true) {
  const status = await client.tenants.status({ tenant_id: tenant_id });
  if (status.data.infra.ready_for_ingestion) break;
