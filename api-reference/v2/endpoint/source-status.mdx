...
| Name | Description |
| --- | --- |
| <Field name="ids" type="string[]" required /> | One or more `id` values returned at ingestion. Accepts IDs for documents, app sources, or memories. Provide multiple IDs as repeated params (`ids=a&ids=b`) or comma-joined (`ids=a,b`); the two forms are equivalent and can be mixed. |
