...
> **Cookbook 05** · Advanced · Sales & GTM
...
graph LR
    ...
    B -->|"POST /ingestion/upload_knowledge<br/>multipart form-data"| C["HydraDB<br/>tenant: competitive-intel<br/>sub_tenant: acme-corp"]
    D["Analyst / Agent"] -->|"POST /recall/full_recall<br/>query + recency_bias"| C
...
def ingest_press_release(competitor: str, title: str, text: str) -> str:
    ...
    resp = requests.post(
        ...
        headers={"Authorization": f"Bearer {API_KEY}"},   # no Content-Type - let requests set multipart boundary
        files={"files": (filename, content.encode("utf-8"), "text/plain")},
        data={"tenant_id": TENANT_ID, "sub_tenant_id": competitor},
    )
    resp.raise_for_status()
    source_id = resp.json()["results"][0]["source_id"]
...
def print_results(result: dict) -> None:
    ...
    for chunk in chunks:
        fname = chunk.get("document_metadata", {}).get("filename", "unknown")
...
|--------|-------------------|------------------|-------|
| Time to answer "what is X doing now?" | 30–60 min (manual) | &lt;10 seconds | **200x faster** |
...
| P95 query latency | N/A (manual) | &lt;200ms | **Sub-second** |
