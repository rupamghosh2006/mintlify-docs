...
description: "Learn how to build a comprehensive workplace search and AI assistant platform using HydraDB APIs. This guide covers data ingestion, search capabilities, and AI-powered Q&A across multiple data sources."
---

> **Status**: This guide is in progress.

This guide will walk you through building an extremely powerful workplace search and AI assistant platform that rivals Glean using HydraDB APIs. You'll learn how to create a unified search experience across multiple data sources with AI-powered Q&A capabilities.
...
graph TD
    ...
    C["HydraDB APIs<br/>• QnA Engine<br/>• Document Index<br/>• Memory Store"]
...
```javascript
class HydraDBDataIngestion {
  constructor(apiKey, tenantId) {
    this.apiKey = apiKey;
    this.tenantId = tenantId;
    this.baseUrl = 'https://api.hydradb.com'; // HydraDB API base URL
  }

  async uploadBatch(sources, subTenantId = null) {
    // Ensure each source has tenant_id and sub_tenant_id
    const appKnowledge = sources.map(source => ({
      ...source,
      tenant_id: this.tenantId,
      sub_tenant_id: subTenantId || this.tenantId
    }));

    const response = await fetch(`${this.baseUrl}/ingestion/upload_knowledge`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ app_knowledge: appKnowledge })
    });
  ...
  async verifyProcessing(fileId) {
    const url = `${this.baseUrl}/ingestion/verify_processing?file_ids=${fileId}&tenant_id=${this.tenantId}`;
  ...
  async uploadDocuments(files, fileMetadata = []) {
...
class GleanSearch {
  ...
  async search(query, options = {}) {
    ...
    const response = await fetch(`${this.baseUrl}/recall/full_recall`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.apiKey}`,
        'Content-Type': 'application/json',
...
class AdvancedSearch extends GleanSearch {
  async searchWithFilters(query, filters = {}) {
    ...
    // Build metadata filter (HydraDB only supports source_title and source_type)
    const metadata = {};
  ...
  async searchWithContext(query, additionalContext = '') {
    // Use additional_context parameter to guide retrieval
    return this.search(query, {
  ...
  async conversationalSearch(query, conversationHistory = []) {
    // Provide conversation history as additional context
    return this.search(query, {
      additionalContext: `Previous conversation context: ${JSON.stringify(conversationHistory)}`
    });
  }
}
```
...
class PersonalizedSearch extends AdvancedSearch {
  ...
  async searchWithMemory(query, userId, options = {}) {
    ...
    const personalizedOptions = {
      ...options,
      additionalContext: this.buildPersonalizedInstructions(userProfile, query)
    };

    // Add user's preferred mode
    if (userProfile.preferredMode) {
      personalizedOptions.mode = userProfile.preferredMode;
    }
    ...
    const searchResults = await this.search(query, personalizedOptions);
  ...
  async getUserProfile(userId) {
    ...
    // Note: HydraDB automatically manages user memories via userName and sessionId
    // This is for additional customization beyond what HydraDB provides
  ...
  async updateUserProfile(userId, query, searchResults) {
    ...
    profile.searchHistory.push({
      ...
      resultCount: searchResults.chunks?.length || 0
    });
    ...
    if (searchResults.chunks) {
      searchResults.chunks.forEach(chunk => {
        if (chunk.source && !profile.favoriteSources.includes(chunk.source)) {
          profile.favoriteSources.push(chunk.source);
        }
      });
