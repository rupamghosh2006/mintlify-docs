...
> **Important:** Prefer the v2 surface in this guide. Older v1 routes like `/recall/full_recall`, `/ingestion/upload_knowledge`, `/list/data`, and `/tenants/create` may still appear in the OpenAPI file for compatibility, but new integrations should use the consolidated v2 endpoints: `/tenants`, `/context/*`, and `/query`.
...
Core raw HTTP responses (`/tenants`, `/context/*`, and `/search`) are wrapped:
...
| Ingest files, app sources, memories | `POST /context/ingest` · `client.context.ingest()` |
...
2. **Ingestion**: after `POST /context/ingest`, poll `GET /context/status` until the source is searchable or fully complete.
...
{
  "relations": {
    "ids": ["related-source-1", "related-source-2"],
...
- Prefer `ids`; legacy alias `cortex_source_ids` is accepted but should not be used in new code.
...
while True:
    ...
    if status.get("indexing_status") in ("graph_creation", "completed"):
        break
    if status.get("indexing_status") in ("errored", "failed"):
        raise RuntimeError(status.get("error_message", "Indexing failed"))
    time.sleep(2)
...
while (true) {
  ...
  if (["graph_creation", "completed"].includes(status.indexing_status as string)) break;
  if (["errored", "failed"].includes(status.indexing_status as string)) {
    throw new Error((status.error_message as string) ?? "Indexing failed");
  }
...
| Field | Purpose |
|---|---|
| `id` | stable source ID / upsert key |
...
while (true) {
  ...
  if (["graph_creation", "completed"].includes(status.indexing_status as string)) break;
  if (["errored", "failed"].includes(status.indexing_status as string)) {
    throw new Error((status.error_message as string) ?? (status.message as string) ?? "Indexing failed");
  }
...
const sources = await client.context.list({
  ...
  pageSize: 50,
  filters: {
    tenantMetadata: { department: "support" },
    additionalMetadata: { author: "alice" },
    sourceFields: { type: "slack" },
  },
});
```
...
- [ ] Using v1 endpoints for new code instead of `/tenants`, `/context/*`, `/query`.
...
|---|---|---|
| `client.context.ingest()` | `POST /context/ingest` | Ingest files, app sources, or memories |
...
|---|---|---|---|
| Context | `client.context.ingest()` | `POST /context/ingest` | Ingest files, app sources, or memories |
