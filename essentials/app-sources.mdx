...
- [TL;DR field reference](#tldr-field-reference): all allowed app-source fields, when to use them, and how they affect ingestion/search.
...
An app source uses the public app-source shape:
...
| `attachments` | Optional | Files attached to this app item. | [Attachments and comments](#attachments-and-comments) |
| `comments` | Optional | Comments or replies attached to this app item. | [Attachments and comments](#attachments-and-comments) |

The example below is a valid app-source object for a Slack message:
...
|---|---|
| Account -> deal/contact | `parent_id` |
| Deal owner | Put in `data.owner` or `metadata.owner` |
...
Search can use comment authors, comment vectors, attachment vectors, and shared-attachment graph edges as extra recall signals.
...
| Using `parent_id` for every relation | Use `parent_id` only for message reply parent or true hierarchy; use `relations` for peers/blockers/custom links |
...
## TL;DR field reference {#tldr-field-reference}
...
| `parent_id` | String | Optional | Parent external ID for custom hierarchies, e.g. account -> deal. | Enables parent/child traversal. |
