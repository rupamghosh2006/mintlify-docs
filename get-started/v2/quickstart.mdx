...
# auth + the v2 header are both wired up.
...
Ingestion is asynchronous(runs in background). Content passes through this pipeline(Queued --> Processing --> Graph Creation --> Completed) before it becomes fully recallable:
...
- **`mode: "thinking"`**  - ensures more accurate results. Higher quality. Use for customer-facing answers. The default `"fast"` skips expansion. See [Recall](/essentials/v2/recall) for the full deep-dive.
