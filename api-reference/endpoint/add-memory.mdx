...
<Warning>
For memories, `tenant_metadata` and `document_metadata` are JSON strings. JSON.stringify them before sending.
</Warning>
...
<Info>
**TODO (SDK):** Confirm the exact property names your TypeScript/Python SDK exposes for OpenAPI `MemoryItem.tenant_metadata` and `MemoryItem.document_metadata` (for example snake_case vs camelCase) and whether values must be pre-stringified. Examples above follow [`api-reference/openapi.json`](/api-reference/openapi.json) (`MemoryItem`).
</Info>
...
| `tenant_metadata` | string | `""` | JSON string of tenant-level metadata (per OpenAPI `MemoryItem`). Use `JSON.stringify` / `json.dumps` before sending. |
| `document_metadata` | string | `""` | JSON string of document-level metadata (per OpenAPI `MemoryItem`). Use `JSON.stringify` / `json.dumps` before sending. |
