...
description: "Quick reference for search modes, type selection, and when to call each."
---

import { Field } from "/snippets/field.jsx";

Use this page to choose the right search shape before opening the full [Search](/api-reference/v2/endpoint/search) endpoint reference. Search has three main decisions: what to search (`type`), how to match (`search_by`), and how much retrieval work to spend (`mode`).
...
| <Field name="search_by" /> | `"hybrid"`, `"text"` | Choose the matching method. Use `"hybrid"` by default and `"text"` for exact terms or phrases. |
...
| <Field name="search_apps" /> | boolean | Adds app-aware retrieval while still searching the full selected knowledge scope. Use it for better app-source matching; it does not restrict retrieval to app sources only. |

<Tip>
For filter design, read [Usage - Metadata](/essentials/v2/metadata) before creating tenant schemas. For exact request fields, defaults, and response shape, use [Search](/api-reference/v2/endpoint/search).
</Tip>
...
|---|---|
| Fast document RAG | `type="knowledge"`, `search_by="hybrid"`, `mode="fast"`, `max_results=5-10`, `graph_context=false` |
| Highest-quality document RAG | `type="knowledge"`, `search_by="hybrid"`, `mode="thinking"`, `graph_context=true`, `alpha="auto"` |
| Personalized answer | `type="all"`, include `sub_tenant_id`, `search_by="hybrid"`, `mode="thinking"` |
| User preferences only | `type="memory"`, include `sub_tenant_id`, `search_by="hybrid"` |
| Exact keyword or phrase | `type="knowledge"`, `search_by="text"`, `operator="phrase"` |
...
- [Usage - Search](/essentials/v2/search) - conceptual overview, retrieval modes, and ranking behavior
