...
| Filter category | cURL / Python key | TypeScript `ContentFilter` key | Targets |
|---|---|---|---|
| Tenant-level metadata | `metadata` | `tenant_metadata` | Fields defined in `tenant_metadata_schema` |
| Document-level metadata | `additional_metadata` | `document_metadata` | Fields attached per-document at ingestion |

<Info>
**TypeScript SDK uses different filter key names.** The `ContentFilter` type exposes `tenant_metadata` and `document_metadata`. The REST API and Python SDK use `metadata` and `additional_metadata`. Both target the same underlying data.
</Info>
| `source_fields` | Top-level source fields | See below |
