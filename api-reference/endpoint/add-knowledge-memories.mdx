...
2. **App Knowledge Uploads**: Upload structured knowledge from workplace apps using the `app_sources` multipart form field with an AppKnowledgeModel JSON array
...
<Warning>
  **Required Fields**: The `tenant_id` and `sub_tenant_id` fields are required for all app knowledge. Ensure these are provided for each item in the `app_sources` array.
</Warning>
...
**For App Knowledge Uploads:**
- **Batch Uploads**: For multiple knowledge sources, include them in a single `app_sources` array
- **Metadata Consistency**: Use consistent metadata schemas across your organization
