...
### Linear ticket with comment attachments

Represent the Linear issue as a `ticket`. Comments go in `comments[]`; files mentioned or uploaded in a comment go in top-level `attachments[]`. Add `properties.comment_id` as a stable traceability convention when the file came from a specific comment.

<Accordion title="Linear comment attachment payload">
  ```json
  {
    "id": "linear_AUTH-123",
    "tenant_id": "acme",
    ...
    "fields": {
      "kind": "ticket",
      "title": "Login redirect loop after token refresh",
      "description": "Users are redirected between /login and /callback.",
      "status": "in_progress",
      "priority": "high",
      "assignee": "alice",
      "reporter": "bob",
      ...
      "url": "https://linear.app/acme/issue/AUTH-123"
    },
    "comments": [
      {
        "id": "lin-comment-1",
        ...
        "properties": {
          "linear_comment_id": "lin-comment-1"
        }
      }
    ],
    "attachments": [
