...
| --- | --- | --- |
| <Field name="metadata" type="object" /> | Source's `metadata` payload | Keys must be declared in the tenant's `tenant_metadata_schema` with `enable_match: true`. Undeclared keys are silently ignored. |
| <Field name="additional_metadata" type="object" /> | Source's `additional_metadata` payload | Free-form per-document JSON. No schema declaration required. |
...
Allowed values are `title`, `type`, `description`, `url`, `timestamp`, `metadata`, `additional_metadata`, and `chunk_count`. Omit or pass `null` to return everything.
