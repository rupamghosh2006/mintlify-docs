...
graph LR
    ...
    E -->|"query() with source: memories"| F["Manager dashboard"]
```
...
def upload_doc(filepath: str, doc_id: str, doc_type: str, team: str):
    ...
    additional_metadata must be a JSON string, not a dict.
    """
...
def ingest_people(people: list):
    ...
    try:
        result = client.context.ingest(
            tenant_id=TENANT_ID,
            files=file_handles,
            additional_metadata=json.dumps(metadata_list),
        )
...
| `422 Unprocessable Entity` on upload | Sending JSON text instead of files | Use `client.context.ingest(files=[...])` - not raw JSON body. |
...
| `client.context.ingest(tenant_id=..., files=..., additional_metadata=...)` | Upload company knowledge documents |
| `client.context.ingest(
...
| `additional_metadata` | `str` | Optional. JSON **string** (not dict) with per-file `id` and `additional_metadata`. |
