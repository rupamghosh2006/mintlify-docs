...
4. **Make it searchable**  -  later, [`POST /search`](/api-reference/v2/endpoint/search) can retrieve it with `type: "memory"` or `type: "all"`.
...
Memories live in their own store, separate from [Knowledge](/essentials/v2/knowledge). The [`POST /search`](/api-reference/v2/endpoint/search) endpoint reaches each store through the `type` parameter:
...
<Info>
  ...
  <p>For personalized responses grounded in shared context, use <code>type: "all"</code> on a single <a href="/api-reference/v2/endpoint/search"><code>POST /search</code></a> call. HydraDB runs both stores in parallel and merges results for you. See <a href="/essentials/v2/search">Search</a>.</p>
</Info>
...
Poll [`GET /source/status`](/api-reference/v2/endpoint/sources-overview) with the returned `source_id` until `indexing_status` is `completed`. You can also use webhooks when available. After indexing, the memory can surface in [`POST /search`](/api-reference/v2/endpoint/search-overview) when called with `type: "memory"` or `type: "all"`.
...
Later, when [`POST /search`](/api-reference/v2/endpoint/search) runs with `type: "memory"` and a query like `"what UI settings does the user prefer?"`, HydraDB can return the inferred preference  -  for example, `"prefers dark mode"`  -  rather than the raw event log.
