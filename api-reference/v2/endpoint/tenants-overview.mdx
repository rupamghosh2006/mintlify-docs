...
1. Create the tenant.
2. Wait for readiness: `scheduler_status`, `graph_status`, `vectorstore_status.knowledge`, and `vectorstore_status.memory` must all be `true`.
...
|---|---|---|
| <Field name="name" type="string" /> | The metadata key you will send during ingestion and use in filters. | `department`, `region`, `customer_tier` |
| <Field name="data_type" type="enum" /> | The value type. Friendly names like `string`, `integer`, `float`, `boolean`, `array`, and `object` are accepted. | Use `string` for categories and labels. |
| <Field name="enable_match" type="boolean" /> | Enables exact filtering on this field. | Filter search to `department = "engineering"`. |
| <Field name="enable_dense_embedding" type="boolean" /> | Enables semantic search over this field. String fields only. | Match meaning across product descriptions. |
| <Field name="enable_sparse_embedding" type="boolean" /> | Enables keyword/BM25 search over this field. String fields only. | Find exact terms in long descriptions. |
