...
- **Tenant-level:** top-level keys (except the reserved nested object below) must match tenant metadata field names declared with **`enable_match`** in `tenant_metadata_schema`; values use **plain equality** to stored tenant metadata.
- **Document-level:** nest an object under the key **`additional_metadata`**; keys and values use **plain equality** to per-source document metadata.
