...
| Task | Endpoint |
| :-- | :-- |
| Upload PDFs, DOCX, CSVs, and other documents HydraDB should parse | `/context/ingest` with `type=knowledge` and `files` |
...
<Note>
  **Why both** `type=knowledge `**and** `app_knowledge`**?** They have a theoretical differentiation.
  ...
  - Within `type=knowledge`, you pick the **payload shape**: `files` (binary documents HydraDB will parse  -  PDFs, DOCX, CSV) or `app_knowledge` (a JSON array of already-extracted content from your app  -  Slack messages, Notion pages, web pages). You can send both in the same request.
</Note>
...
- **Source IDs**: Unique identifiers returned by `/context/ingest`. You can assign custom IDs using `id` in metadata or `id` in app knowledge. Use them for polling status, inspecting content, and deleting sources.
...
result = client.context.ingest(
    type="knowledge",
    tenant_id="acme_corp",
    files=[("runbook.pdf", f, "application/pdf")],
    document_metadata=json.dumps([{
        "id": "runbook_deploy",
        "tenant_metadata": {"department": "ops"},
        "document_metadata": {"owner": "platform-team"},
