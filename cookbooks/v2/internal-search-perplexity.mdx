...
```bash
curl -X POST \
  'https://api.hydradb.com/context/status?tenant_id=company-knowledge&ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
| `POST` | `/context/status?tenant_id=...&ids=...` | Check indexing status |
...
```
GET /context/status?tenant_id=company-knowledge&ids=YOUR_SOURCE_ID
Body: {}
```
