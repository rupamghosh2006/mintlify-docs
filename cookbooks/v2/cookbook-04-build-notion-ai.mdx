...
def upload_batch(sources: list, sub_tenant_id: str = None) -> list:
    ...
    return [r.id for r in (result.results or []) if r.id]
...
**Endpoint:** `GET /context/status?ids=SOURCE_ID&tenant_id=TENANT` - Check indexing status
...
**POST** `/context/status?ids=SOURCE_ID&tenant_id=YOUR_TENANT` - Poll until status = "completed"
