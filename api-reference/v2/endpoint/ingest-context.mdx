...
description: "Ingestion endpoint for knowledge (files, app sources) and user memories."
...
<Accordion title="Knowledge documents - upload PDFs, DOCX, CSV, markdown, or text">
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="id" type="string" /> | Optional source ID. If set, becomes the `id` for this file (use your app's document ID for parity). |
  ...
  | <Field name="relations" type="object" /> | Declare forceful relations to other sources. Shape: `{ "ids": ["...", "..."] }` (legacy alias `cortex_source_ids` still accepted). Surfaced via `additional_context` in `mode: "thinking"` search. |
</Accordion>

<Accordion title="App knowledge - index text your workspace or personal apps">
  <CodeGroup>
...
<Accordion title="3. User memories - store notes or infer user preferences">
  ...
  | <Field name="relations" type="object" /> | Forceful relations within the Memories store. Shape: `{ "ids": ["...", "..."] }` (legacy alias `cortex_source_ids` still accepted). |
</Accordion>
...
- **Async indexing.** `202 Accepted` means HydraDB queued the work, not that content is searchable. Poll [Source Status](/api-reference/v2/endpoint/source-status) until `indexing_status` reaches `graph_creation` (searchable) or `completed` (graph-ready).
