...
description: "Official TypeScript/Node.js and Python SDKs for the HydraDB platform. Build powerful, context-aware AI applications."
---
...
<Tabs>
  <Tab title="TypeScript">
    ```ts
    import { HydraDBClient } from "@hydradb/node";
  ...
  <Tab title="Python">
    ```python
    import os
    from hydradb import HydraDB, AsyncHydraDB
...
<Info>
  **Python**: We provide both synchronous and asynchronous clients. Use `AsyncHydraDB` for async/await patterns and `HydraDB` for traditional synchronous operations.
</Info>
...
When you index your data, you make it ready for retrieval from HydraDB using natural language.
...
> **For a more detailed explanation** of document upload, including supported file formats, processing pipeline, metadata handling, and advanced configuration options, refer to the [Ingest Knowledge](https://docs.hydradb.com/api-reference/endpoint/add-knowledge-memories).
...
<Tabs>
  <Tab title="TypeScript">
    ```ts
    // Hybrid search over indexed sources (documents)
    ...
    // Hybrid search over user memories
  ...
  <Tab title="Python (Async)">
    ...
    async def search_sources():
        return await async_client.recall.full_recall(
            query="Which mode does user prefer",
            tenant_id="tenant_1234",
            sub_tenant_id="sub_tenant_4567",
            alpha=0.8,
    ...
    async def search_preferences():
        return await async_client.recall.recall_preferences(
            query="Which mode does user prefer",
            tenant_id="tenant_1234",
            sub_tenant_id="sub_tenant_4567",
            alpha=0.8,
