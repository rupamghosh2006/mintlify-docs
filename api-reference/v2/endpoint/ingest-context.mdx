...
- **Declare schema fields upfront.** Keys not in `tenant_metadata_schema` are silently ignored at query time. Define filterable fields when creating the database via [Create Tenant](/api-reference/v2/endpoint/create-tenant).
...
- **Collection defaulting.** Omitting `collection` writes to the default collection. List available collections with [List Sub-Tenants](/api-reference/v2/endpoint/list-sub-tenants).
