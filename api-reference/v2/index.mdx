...
| [Memory](essentials/v2/memory) | User-specific context such as preferences, conversation history, notes, and inferred traits. | Use `type=memory` when the content should personalize answers for a specific user or sub-tenant. |
...
**Response envelope:** Raw HTTP responses are wrapped in a consistent envelope. Endpoint pages document the resource-specific `data` payload unless the full envelope is important to the behavior.
