...
`GET /tenants/status?database=...` · `client.tenants.status()`
...
| `DELETE /tenants?database=...` | delete a tenant and all data |
| `GET /tenants/sub-tenants?database=...` | list active sub-tenants |
| `GET /tenants/stats?database=...` | collection counts and vector dimensions |
...
curl -s "https://api.hydradb.com/tenants/status?database=${DATABASE}" \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY" \
