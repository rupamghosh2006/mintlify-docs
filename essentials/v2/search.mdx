...
| I want to… | `type` | `search_by` | `mode` | Notes |
| --- | --- | --- | --- | --- |
...
| Search over apps | `"knowledge"` | `"hybrid"` | `"thinking"` | Set `search_apps: true` to enable the app-aware retrieval lane (IDs, actors, thread & parent traversal) while still searching the full selected knowledge scope |
...
| `search_apps` | boolean | Set `true` to enable the app-aware retrieval lane alongside normal retrieval (reconstructed threads, parent/child traversal, exact ID/actor lookups). This improves app-source search but does not restrict the query to only app sources; HydraDB still searches the full selected knowledge scope. See [App Sources](/essentials/v2/app-sources). Default: `false`. |
...
- `search_apps`  -  Set to `true` when querying ingested app data (Slack, Gmail, Confluence, Jira, Salesforce) to activate exact ID and actor lookups, same-thread expansion, and parent/child hierarchy traversal. This adds better app-aware retrieval on top of the normal knowledge search; it does not filter out non-app knowledge. Pair with `mode: "thinking"`.

---

## 4. Minimal working example

The canonical personalized-answer flow takes a single call: `POST /search` with `type: "all"` returns merged knowledge and per-user memory in one ranked result set.
...
If you need to keep the two streams separate  -  for instance to format user preferences differently from documents in the prompt  -  call `/search` twice in parallel with `type: "knowledge"` and `type: "memory"`, then merge client-side.
...
| Memories missing from a `type: "knowledge"` search | Wrong store selected | Use `type: "memory"` or `type: "all"`. |
...
**Cache the prompt context.** If the same query repeats inside a session, cache the `RetrievalResult` keyed by `(tenant_id, sub_tenant_id, query, type, search_by)`. Recall is fast, but skipping it entirely is faster.
