...
`POST /query` is the single search endpoint. Two parameters decide what runs:
...
That is why HydraDB exposes semantic retrieval through `query_by: "hybrid"` inside the unified `/search` endpoint rather than as a separate pure-vector mode.
...
Start with the API default (`0.8`) and tune from observed results. If users search for exact IDs and get loosely related content, lower `alpha`. If they ask broad conceptual questions and get sparse results, raise it. `alpha` applies only to `query_by: "hybrid"`; it is ignored for `"text"`.
