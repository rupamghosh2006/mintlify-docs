...
Applications commonly define several stable fields they expect to filter on. Use `INT32`, `INT64`, `BOOL`, `FLOAT`, or other typed fields when the value is not a string. Every filterable field needs `enable_match: true`.
...
**Matchable vs non-matchable fields**

Only fields declared with `enable_match: true` should be relied on for deterministic filtering. A field without `enable_match` can still be stored, but should not be relied on for deterministic filtering through `metadata_filters`.
...
- `region` can be used in `metadata_filters`.
- `internal_notes` can be stored as tenant metadata, but should not be relied on for deterministic filtering unless match-enabled.
...
**Embeddings-enabled text field**

Embedding flags are for retrieval over text content, not for exact-match metadata filtering. Use them when the field itself is a body of searchable text – for example, a description that you want included in semantic or keyword recall.
