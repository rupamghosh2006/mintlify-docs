---
title: "Building an AI Travel Planner"
...
Start by uploading comprehensive hotel and accommodation data using HydraDB's batch upload capabilities:
...
const uploadHotelData = async (hotels) => {
  ...
  await hydradb.uploadBatch(hotelSources);
};
```
...
const uploadFlightData = async (flights) => {
  ...
  await hydradb.uploadBatch(flightSources);
};
```
...
const uploadRestaurantData = async (restaurants) => {
  ...
  await hydradb.uploadBatch(restaurantSources);
};
```
...
const uploadActivityData = async (activities) => {
  ...
  await hydradb.uploadBatch(activitySources);
};
```
...
class TravelAssistant {
  ...
  async planTrip(query, userProfile) {
    // Use HydraDB's multi-step reasoning for complex travel planning
    const response = await this.hydradb.qna({
      question: query,
...
class PersonalizationEngine {
  ...
  async generateUserMemory(userInteraction) {
    ...
    await this.hydradb.generateUserMemory({
  ...
  async getPersonalizedRecommendations(userId, destination, userName) {
    ...
    const memories = await this.hydradb.retrieveUserMemory({
...
const searchTravelExperiences = async (query, filters = {}) => {
  ...
  const response = await hydradb.qna({
...
const analyzeBookingPatterns = async (userId, bookingData) => {
  ...
  await hydradb.generateUserMemory({
    tenant_id: userId,
