...
3. **Filters third.** Are you using the right namespace? Top-level `metadata_filters` keys match `metadata`; free-form fields belong under `additional_metadata`. For hot top-level filters, declare the field in `database_metadata_schema` with `enable_match: true`. See [Metadata](/essentials/v2/metadata).
...
- [Core Concepts](/get-started/v2/core-concepts)  -  the five primitives: Memories, Knowledge, Query, Databases, Metadata
