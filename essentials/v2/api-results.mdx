...
`POST /search` returns structured JSON. Before you can pass it to an LLM, you need to convert it into a plain string. This page shows how.

The same pattern works regardless of whether you set `source: "knowledge"`, `source: "memories"`, or `source: "all"` - the response shape is identical.
...
The simplest path is one `POST /search` with `source: "all"` - HydraDB searches both stores in parallel and returns one merged, ranked result set.
...
- **`source: "all"` is the simplest path when you need both knowledge and memories.** One call, one result set, one `build_string` call.
