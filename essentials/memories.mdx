---
title: "Memories"
description: "Memories are the fundamental building blocks of HydraDB. They form the context store that powers intelligent, personalized, and adaptive AI agents."
---
...
HydraDB treats all of this as **living memory**, not static data. Every memory is parsed, enriched, connected, and continuously optimized for recall.
...
Unlike traditional memory systems (logs, chat history, or static profiles), HydraDB memories are **dynamic state**.
...
HydraDB supports multiple classes of memory, all unified under a single system.
...
The real power of HydraDB memories comes from compounding.

HydraDB doesn’t just store information — it **learns from interaction**.
...
Over time, HydraDB builds a **behavioral model per user and per agent**.
...
HydraDB doesn’t store isolated vectors — it builds a **context graph** connecting:
...
At query time, HydraDB retrieves memories using its multi-stage recall engine:
...
HydraDB handles lifecycle management so memory remains **clean, relevant, and useful**.
...
After a few interactions, HydraDB learns:
...
```
curl -X POST https://api.hydradb.com/memory/recall \
  -H "Authorization: Bearer <API_KEY>" \
  -d '{
    "tenant_id": "demo",
    "user_id": "john",
...
Without HydraDB, you would need to build:
...
HydraDB gives you all of this as **infrastructure**.
