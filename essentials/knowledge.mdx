...
```bash cURL
curl -X POST \
  'https://api.hydradb.com/ingestion/verify_processing?file_ids=policy_v2&tenant_id=acme_corp&sub_tenant_id=default' \
  -H "Authorization: Bearer $HYDRA_DB_API_KEY"
```
```typescript TypeScript SDK
...
| `sub_tenant_id` | string | Required sub-tenant scope. Use `"default"` if you do not partition knowledge. |
