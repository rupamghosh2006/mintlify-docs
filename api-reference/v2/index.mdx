...
| [`/search`](/api-reference/v2/endpoint/search) | `POST` | `search.query` | Unified search over knowledge, memories, or both | You need retrieval with `hybrid` or `text` search across `knowledge`, `memories`, or `all`. |
...
- **Search modes.** `POST /search` supports `search_by: "hybrid"` or `"text"` and `type: "knowledge"`, `"memory"`, or `"all"`.
