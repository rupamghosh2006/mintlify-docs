...
A single method covers all retrieval. Pick `type` (`"knowledge"`, `"memory"`, `"all"`) and `search_by` (`"hybrid"`, `"text"`, `"vector"`) to control behavior.
...
A single method covers every retrieval pattern  -  switch `type` and `search_by` to control behavior:
...
- **Enum-typed values** for `type`, `search_by`, `operator`, `mode`, and `type`
</Check>
