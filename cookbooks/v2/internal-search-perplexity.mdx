...
- **User Memory**: Per-user profiles stored via `POST /source/ingest` and retrieved via `POST /query` to personalize answer depth and format.
...
> **Note**: [`GET /source/status`](/api-reference/v2/endpoint/source-status) takes `source_ids` and `tenant_id` as **query parameters**. Pass multiple `source_ids` to check a batch in one call.

```bash
curl -X POST \
  'https://api.hydradb.com/source/status?tenant_id=company-knowledge&source_ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
def get_user_profile(user_id: str) -> str:
    """Retrieve a user's stored memory profile via /query with source: "memories"."""
    result = client.query(
        source='memories',
        tenant_id=TENANT_ID,
        sub_tenant_id=f"user-{user_id}",
...
def explain_decision(question: str, user_id: str) -> str:
    ...
    Step 1: search chunks + graph relations from /query (source="knowledge").
    Step 2: retrieve user profile from /query (source="memories").
...
> **Reading the response**: [`/query`](/api-reference/v2/endpoint/query) returns the same structure regardless of `source`  -  read the profile from `chunks[0].chunk_content`. `additional_metadata` will be `null` for memory entries; that's expected.
...
| `POST` | `/source/status?tenant_id=...&source_ids=...` | Check indexing status |
...
```
GET /source/status?tenant_id=company-knowledge&source_ids=YOUR_SOURCE_ID
Body: {}
```
