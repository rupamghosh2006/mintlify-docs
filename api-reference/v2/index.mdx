...
| [Search](/api-reference/v2/endpoint/search-overview) | Retrieve context with hybrid or text search | At query time  -  the only endpoint you call to feed an LLM |
...
| [`/search`](/api-reference/v2/endpoint/search) | `POST` | `search.query` | Unified search over knowledge, memories, or both | You need retrieval with `hybrid` or `text` search across `type: "knowledge"`, `type: "memory"`, or `type: "all"`. |
...
- **Search modes.** `POST /search` supports `search_by: "hybrid"` or `"text"` and `type: "knowledge"`, `"memory"`, or `"all"`. The same `type` enum is used across ingestion, listing, deletion, and search; search additionally accepts `"all"`.
...
- **Go deeper:** [Usage](/essentials/v2/search) covers each primitive in depth
