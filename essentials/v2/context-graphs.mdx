...
When `graph_context: true` is set on a search call (the default), HydraDB returns relationship data alongside the retrieved chunks - showing how those chunks connect to each other and to your query. Set `graph_context: false` to drop the graph slice when you only need ranked chunks.
...
**At search**, when `graph_context: true` is set (the default):
...
For full field schemas, see the [Search API Reference](/api-reference/v2/endpoint/query-overview).
...
- [Full Search API Reference](/api-reference/v2/endpoint/query-overview) - full graph response schema
