---
openapi: "POST /recall/full_context_recall"
title: "Full-context Recall"
description: "Hybrid search over indexed sources (documents) only. Use this when you need context from your knowledge base or uploaded documents."
---

### When to use it

Use **full-context recall** when you want to search over **indexed knowledge** (documents, knowledge base, uploaded files). Same request body as [recall preferences](/api-reference/endpoint/recall-preferences) (memories); the endpoint you call determines the search target.
...
<Tabs>
  <Tab title="API Request">
    ```bash
    curl -X 'POST' \
    'https://api.usecortex.ai/recall/full_context_recall' \
  ...
  <Tab title="TypeScript">
    ```ts
    const results = await client.recall.fullContextRecall({
  ...
  <Tab title="Python (Sync)">
    ...
    results = client.recall.full_context_recall(
        query="Which mode does user prefer",
