...
```bash
curl -X POST \
  'https://api.hydradb.com/context/status?tenant_id=competitive-intel&ids=YOUR_SOURCE_ID' \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
def wait_until_indexed(id: str, max_tries: int = 20, interval: int = 3) -> None:
    """
    Poll /context/status until the source is indexed or times out.
...
```
GET /context/status?tenant_id=competitive-intel&ids=YOUR_SOURCE_ID
Body: {}
```
