...
The same pattern works for every call to [`POST /search`](/api-reference/v2/endpoint/search-overview), whether you set `type: "knowledge"`, `type: "memory"`, or `type: "all"`  -  the response shape is identical.

---

## 1. What you get back

`POST /search` returns the same core response shape regardless of `type` or `search_by`:
...
For personalised answers, the simplest path is one `POST /search` with `type: "all"`  -  HydraDB searches both stores in parallel and returns one merged, ranked result set.
...
knowledge_result, memory_result = await asyncio.gather(
    async_client.search.query(
        ...
        type="knowledge", search_by="hybrid", mode="thinking",
    ),
    async_client.search.query(
        tenant_id="acme_corp", sub_tenant_id="user_123",
        query="answer style preferences",
        type="memory", search_by="hybrid",
    ),
)
...
| Ignoring Memories when personalisation matters | Answers feel generic | Use `type: "all"` on `POST /search` (or call `type: "memory"` separately) and include the result in the prompt. |
...
| Disabling graph fields and then expecting them | Graph context will be omitted from the response | `graph_context: true` is the default - only set it to `false` when you explicitly don't want graph data. |
...
- [Memories](/essentials/v2/memories)  -  what `POST /search` with `type: "memory"` searches
- [Knowledge](/essentials/v2/knowledge)  -  what `POST /search` with `type: "knowledge"` searches
