...
> **Goal**: Ingest six source types into one HydraDB tenant, store per-user memory profiles for personalized answers, and answer three query patterns - simple Q&A, decision provenance, and cross-source synthesis - all through `POST /recall/full_recall`.
...
> **Important**: The cookbook originally shows `/search/qna` with a `question` field for Q&A queries. This endpoint returns 404. The correct endpoint is `POST /recall/full_recall` with a `query` field. Use this for all three query patterns below.

### 5.1 Simple Q&A - "What is X?" / "How does X work?"
...
def print_results(result: dict) -> None:
    ...
    for chunk in chunks:
        fname = chunk.get("document_metadata", {}) or {}
...
def explain_decision(question: str, user_id: str) -> str:
    ...
    ctx_parts = []
    for c in chunks:
        fname = (c.get("document_metadata") or {}).get("filename", "unknown source")
...
> **Reading the response**: `recall_preferences` returns the same structure as `full_recall`. Read the profile from `chunks[0]["chunk_content"]`. `document_metadata` will be `null` for memory entries - this is expected.
...
| Time saved per complex question | 45 min (manual) | ~8 min | &lt;30 sec | **−94%** |
| P95 query latency | N/A (manual) | 220ms | &lt;200ms | **Sub-second** |
