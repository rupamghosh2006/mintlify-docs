...
| <Field name="graph_context" /> | boolean | When `true` (default), includes the entity/relation graph slice for retrieved chunks in the response (`graph_context` field). Set to `false` to drop it when you only need ranked chunks. (default=`true`) |
