...
```
STATUS CODE: 200
RESPONSE: {"results": [{"id": "YOUR_SOURCE_ID_HERE", "filename": "auth_middleware.md", "status": "accepted", "error": null}]}

Copy your id for use in verify.py: {'results': [{'id': 'YOUR_SOURCE_ID_HERE', 'filename': 'auth_middleware.md', 'status': 'accepted', 'error': None}]}
```
...
Replace `YOUR_SOURCE_ID_HERE` with the `id` from Step 2:
...
def verify_source(id: str, timeout: int = 120, interval: int = 3):
    """
...
STATUS CODE: 200
RESPONSE: {"statuses": [{"id": "YOUR_SOURCE_ID_HERE", "indexing_status": "processing"}]}
  indexing_status: processing - waiting...
STATUS CODE: 200
RESPONSE: {"statuses": [{"id": "YOUR_SOURCE_ID_HERE", "indexing_status": "completed"}]}
...
def upload_batch(batch: list) -> list:
    ...
    ids = [r.id for r in (result.results or []) if r.id]
...
def ingest_pull_requests(prs: list, repo_name: str) -> list:
    ...
    for pr in prs:
        ...
        batch.append({
            ...
            "relations":   {"hydradb_source_ids": ids},
...
async def chat(body: dict):
    ...
    async def stream():
        ...
        sources_list = [{"title": s.title, "id": s.id} for s in (recall_payload.sources or [])]
...
def ask(body: dict):
    ...
    return {
        "answer":        full_answer,
        "sources":       [{"title": s.title, "id": s.id} for s in (recall_payload.sources or [])],
...
**POST** `/context/status` - Returns `statuses[]` array · poll until `"completed"`
