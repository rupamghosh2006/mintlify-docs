...
**Note:** When ingesting, use `metadata` (tenant-level) and `additional_metadata` (document-level) in the request body. In recall responses, these are returned under the same field names: `metadata` and `additional_metadata`.
