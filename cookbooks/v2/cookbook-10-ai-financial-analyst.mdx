...
def ingest_earnings_pdf(
    ...
    """
    Upload a single earnings PDF. Returns the source_id for verification.
    ...
    source_id = result.results[0].source_id if result.results else str(result)
    print(f"Uploaded {ticker} {period_label} {doc_type} → source_id: {source_id}")
    return source_id
...
source_ids = ingest_earnings_batch(filings)
# Output:
# Uploaded ACME Q1 2023 earnings_transcript → source_id: 988293fa-e29
# Uploaded ACME Q2 2023 earnings_transcript → source_id: 9d7951c8-160
# Uploaded ACME Q3 2023 earnings_transcript → source_id: 2cf6ea6e-5fe
# Uploaded ACME Q4 2023 earnings_transcript → source_id: edff67d5-237
```
...
def ingest_board_memo(
    ...
    source_id = result.results[0].source_id if result.results else str(result)
    print(f"Uploaded memo: {ticker} {period_label} → {source_id}")
    return source_id
```
...
def verify_all_indexed(source_ids: list[str], poll_interval: int = 3, max_polls: int = 20) -> bool:
    """
    Poll /source/status until all source_ids report status 'completed'.
...
```http
GET /source/status?source_ids=SOURCE_ID&tenant_id=financial-analyst
```
