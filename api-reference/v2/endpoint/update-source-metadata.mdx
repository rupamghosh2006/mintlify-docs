...
description: "Merge schema-backed metadata and free-form metadata for one source without re-ingesting content."
---
...
| <Field name="tenant_metadata" type="object" /> | Schema-backed metadata fields to merge into the source's `metadata`. The API field is named `tenant_metadata` for compatibility; keys must satisfy the database metadata schema when one exists. |
...
<Warning>
  This edit endpoint intentionally uses the legacy-named `tenant_metadata` field for schema-backed source metadata. The shorter `metadata` field used by ingestion and list examples is not accepted in this PATCH body. `document_metadata` is also not accepted; use `additional_metadata`.
</Warning>
