...
| `query_forceful_relations` | boolean | Whether to fetch author-declared related sources (see `relations`[ on ingest](/api-reference/v2/endpoint/ingest-content)) into `additional_context`. **Only takes effect in** `mode: "thinking"`**.** Default: `true`. |
...
| Recent uploads don't appear in results | Indexing not finished | Poll `GET /context/status/id`  -  chunks are invisible until processing reaches at least `graph_creation`. |
