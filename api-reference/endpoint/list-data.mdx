...
const result = await client.fetch.listData({
  ...
  pageSize: 25,
  filters: {
    tenantMetadata: { category: "engineering" },
    documentMetadata: { author: "Alice Smith" }
  },
...
| Filter category | Targets | Example |
|---|---|---|
| `metadata` | Fields defined in `tenant_metadata_schema` at tenant creation | `{"category": "engineering"}` |
| `additional_metadata` | Fields attached per-document at ingestion | `{"author": "Alice Smith", "year": 2023}` |
