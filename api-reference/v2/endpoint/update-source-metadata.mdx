...
- If an edited tenant metadata field has `enable_dense_embedding` or `enable_sparse_embedding`, HydraDB synchronously refreshes the relevant vector-index metadata search lane.
- If the edited fields are `enable_match`-only, the edit remains database-only and `vector_sync_required` is `false`.
...
| <Field name="vector_rows_synced" type="integer" /> | Number of chunk rows synced to the vector index when sync was required. |
