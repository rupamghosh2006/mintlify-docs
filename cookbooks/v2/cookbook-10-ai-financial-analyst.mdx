...
def ingest_earnings_pdf(
    ...
    id = result.results[0].id if result.results else str(result)
...
def ingest_metrics_series(metrics_by_period: list[dict]) -> list[str]:
    ...
    for item in metrics_by_period:
        ...
        memory_ids.append(result.results[0].id if result.results else "ok")
...
```http
GET /context/status?ids=SOURCE_ID&tenant_id=financial-analyst
```
...
| `file=f` instead of `files=[f]` in SDK | `TypeError` on `context.ingest()` | SDK expects a **list**: `files=[f]` not `file=f` |
