...
HydraDB gives your AI three types of memory — the same way humans remember. Once you understand this model, everything else clicks.
...
```typescript
// Store a user memory
await hydradb.userMemory.store({
  userId: "sarah_123",
...
await hydradb.userMemory.extractFromConversation({
  userId: "sarah_123",
...
```typescript
// Ingest a document
await hydradb.knowledgeMemory.ingest({
  file: policyDocument,
  filename: "refund-policy.pdf"
});

// Connect an app
await hydradb.knowledgeMemory.connectSource({
  source: "notion",
...
```typescript
// Store a hive memory
await hydradb.hiveMemory.store({
...
const playbook = await hydradb.hiveMemory.recall({
...
Recall is how you pull relevant memories at query time. HydraDB searches across all memory types and returns what's most relevant.

```typescript
const context = await hydradb.memory.recall({
  memorySpaceId: "acme-corp",
  userId: "sarah_123",
...
const results = await hydradb.memory.recall({
...
<CardGroup cols={2}>
  ...
  <Card title="Add Knowledge Memories" icon="book" href="/api-reference/endpoint/add-knowledge-memories">
    Ingest documents and apps
  </Card>
  <Card title="Add Hive Memories" icon="hexagon" href="/api-reference/endpoint/add-hive-memories">
