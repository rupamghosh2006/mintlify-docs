...
|---|---|---|
| [Database lifecycle](/api-reference/v2/endpoint/tenants-overview) | Create, monitor, and manage isolated workspaces | First step in an integration, and whenever you need readiness or usage information. |
| [Context lifecycle](/api-reference/v2/endpoint/sources-overview) | Ingest, check indexing, inspect, update, list, and delete Knowledge or Memory | Every time context enters or changes in HydraDB. |
| [Choose a query](/api-reference/v2/endpoint/query-overview) | Retrieve Knowledge, Memory, or both with hybrid or text matching | When you need context to ground an LLM response. |
| [Connector lifecycle](/api-reference/v2/endpoint/connectors-overview) | Connect Slack, Notion, Gmail, GitHub, or Linear and sync selected resources | When HydraDB should continuously pull from an external provider. |
...
| [`/context/sources/{source_id}/metadata`](/api-reference/v2/endpoint/update-source-metadata) | `PATCH` | REST | Update source metadata | You need to merge schema-backed `tenant_metadata` or free-form `additional_metadata` onto one existing source without re-ingesting. |
...
| [`/connectors`](/api-reference/v2/endpoint/create-connector) | `POST` | `connectors.create` | Create a connector | You want HydraDB to sync a provider account. |
| [`/connectors/{id}/discover`](/api-reference/v2/endpoint/discover-connector-resources) | `GET` | `connectors.discover` | Discover connector resources | You need to choose which provider resources to sync. |
| [`/connectors/{id}/configure`](/api-reference/v2/endpoint/configure-connector) | `POST` | `connectors.configure` | Configure connector resources | You are setting selected resources and sync options. |
| [`/connectors/{id}/sync`](/api-reference/v2/endpoint/sync-connector) | `POST` | `connectors.sync` | Sync a connector | You want to trigger an on-demand sync. |
