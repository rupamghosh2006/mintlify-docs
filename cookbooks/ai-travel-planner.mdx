...
> **Note**: All code examples in this guide are for demonstration purposes. They show the concepts and patterns you can use when building your own travel planning application with HydraDB APIs. Follow the [actual API documentation](https://docs.hydradb.com/api-reference/introduction) to adapt these examples to your specific use case, technology stack, and requirements.
...
graph TD
    ...
    C["HydraDB APIs<br/>• QnA Engine<br/>• AI Memories<br/>• Multi-Step Reasoning"]
...
const uploadHotelData = async (hotels, tenantId, subTenantId) => {
  ...
  await client.upload.knowledge({ app_knowledge: hotelSources });
};
```
...
const uploadFlightData = async (flights, tenantId, subTenantId) => {
  ...
  await client.upload.knowledge({ app_knowledge: flightSources });
};
```
...
const uploadRestaurantData = async (restaurants, tenantId, subTenantId) => {
  ...
  await client.upload.knowledge({ app_knowledge: restaurantSources });
};
```
...
const planFamilyVacation = async (query, userProfile) => {
  const response = await client.recall.fullRecall({
    ...
    mode: "thinking",
    max_results: 20,
    additional_context: "Family vacation in Orlando, budget $4000, 7 days, kids aged 8 and 12"
  });

  // Process response to generate structured itinerary
  const itinerary = {
    accommodation: extractHotelRecommendations(response.chunks),
    activities: extractFamilyActivities(response.chunks),
    dining: extractKidFriendlyRestaurants(response.chunks),
    transportation: extractTransportationOptions(response.chunks)
  };
...
const planBusinessTravel = async (query, userProfile) => {
  const response = await client.recall.fullRecall({
    ...
    mode: "fast",
    max_results: 10,
    additional_context: "Business travel to London, needs WiFi and meeting rooms near financial district"
  });

  return processBusinessTravelResponse(response);
};
```
...
const planAdventureTravel = async (query, userProfile) => {
  const response = await client.recall.fullRecall({
    ...
    mode: "thinking",
    max_results: 15,
    additional_context: "Adventure travel to Nepal, trekking, intermediate level, cultural experiences"
  });
