---
title: "Building Your Own Glean with HydraDB"
...
const normalizedData = {
  ...
  hydradb_metadata: {}, // Optional: custom metadata for HydraDB processing
  meta: {
...
class HydraDBDataIngestion {
  ...
  async verifyProcessing(fileId) {
    const url = `${this.baseUrl}/ingestion/verify_processing?file_ids=${fileId}`;
...
class PersonalizedSearch extends AdvancedSearch {
  ...
  async searchWithMemory(query, userId, options = {}) {
    ...
    const personalizedOptions = {
      ...options,
      userName: userId, // HydraDB can automatically manage user memories
...
- **User Names**: Provide consistent user names to enable HydraDB's automatic memory management
...
const SourceCard = ({ source, chunks }) => {
  const sourceChunks = chunks?.filter(chunk => chunk.id === source.id) || [];
...
class DataPrivacyManager {
  ...
  async deleteOldData(tenantId, cutoffDate) {
    ...
    const response = await fetch('https://api.hydradb.com/memory/delete', {
      method: 'DELETE',
      headers: {
...
- **Auto Agent Routing**: Let HydraDB choose the best AI agent for each query
