...
`POST /search` returns structured JSON. Before you can pass it to an LLM, you need to convert the retrieval payload into a plain string. This page shows how.
...
The SDKs return the inner `data` payload from `POST /search`. Raw HTTP responses wrap the same payload under `data` inside the standard response envelope.

The retrieval payload has the same core shape regardless of `type` or `search_by`:
...
Use `build_string` / `buildString` from the SDK. It takes any `POST /search` result and returns a formatted plain string.
...
The simplest path is one `POST /search` with `type: "all"` - HydraDB searches both stores in parallel and returns one merged, ranked result set.
...
If you need to format knowledge and memories in separate labeled sections, call `POST /search` twice in parallel:
...
If memory search fails or times out, fall back to the knowledge-only prompt.
...
If you are not using the SDK, use the helper below to call `POST /search`, format the response, and pass the result to your LLM.
