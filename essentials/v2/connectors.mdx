...
| Provider | Value to use | Example |
|---|---|---|
| Slack | Workspace ID (starts with `T`) | `T12345ACME` |
| GitHub | Organization or user login | `my-github-org` |
| Linear | Workspace name or ID | `acme-linear` |
| Notion | Workspace name or ID | `acme-notion` |
| Gmail | Leave empty — use `account_email` in `additional_metadata` to scope by account instead | |
