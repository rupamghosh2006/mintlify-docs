...
<Note>
  In API requests, a tenant is set with the **`database`** field (formerly `tenant_id`) and a sub-tenant with the **`collection`** field (formerly `sub_tenant_id`). The old names still work as deprecated aliases, so existing code keeps running unchanged.
</Note>
